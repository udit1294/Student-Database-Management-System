...
class Student:
    def __init__(self,root):  #constructor initialize
        ...
        btn_add=Button(button_frame,text='Add',command=self.add_data,font=('arial',12,'bold'),width=14,bg='green',fg='white',cursor='hand2')
        ...
        btn_delete=Button(button_frame,text='Delete',command=self.delete_data,font=('arial',12,'bold'),width=14,bg='red',fg='white',cursor='hand2')
        btn_delete.grid(row=0,column=2,sticky=W,padx=1)

        btn_reset=Button(button_frame,text='Reset',command=self.reset_data,font=('arial',12,'bold'),width=14,bg='yellow',fg='white',cursor='hand2')
        ...
        com_search=ttk.Combobox(search_frame,textvariable=self.var_com_search,font=('arial',12,'bold'),width=20,state='readonly')
        com_search['values']=('Select Option','Roll','Name','Student_id')
        com_search.current(0)
        com_search.grid(row=0,column=1,padx=2,pady=7,sticky=W)

        self.var_search=StringVar()

        txt_search=ttk.Entry(search_frame,textvariable=self.var_search,font=('arial',11,'bold'),width=25)
        txt_search.grid(row=0,column=2,padx=2,pady=7,sticky=W)

        #button key

        btn_Search=Button(search_frame,text='Search',command=self.search_data,font=('arial',12,'bold'),width=15,bg='blue',fg='white',cursor='hand2')
        btn_Search.grid(row=0,column=3,sticky=W,padx=1)

        btn_Showall=Button(search_frame,command=self.fetch_data,text='Showall',font=('arial',12,'bold'),width=15,bg='cyan',fg='black',cursor='hand2')
    ...
    def add_data(self):
        if self.var_dep.get()=='' or self.var_name.get()=='' or self.var_id.get()=='':
            ...
            try:
                ...
                self.fetch_data()
                conn.close()
                messagebox.showinfo('Sucsess','student has been added!',parent=self.root)
   ...
   #Fetch Function
    ...
    def delete_data(self):
        if self.var_id.get()=='':
            ...
            try:
                Delete=messagebox.askyesno('Delete','Are you Sure to update this student data',parent=self.root)
                if Delete>0:
                    conn=mysql.connector.connect(host='localhost',username='root',password='1234',database='sdms')
                    my_cursor=conn.cursor()
    ...
    def open_img(self):
        ...
        self.photoimage_browse=ImageTk.PhotoImage(img_browse)
        self.btn_1.config(image=self.photoimage_browse)
